#!/usr/bin/env zsh
# mpc_live_e2e_test_zsh.sh — End-to-end test using the LIVE Ktor endpoint (no CLI fold)
# Shell: zsh
#
# Flow:
#   1) Wait for server + keys
#   2) Ensure chain is empty (unless ALLOW_DIRTY_CHAIN=1)
#   3) Cast 3 sample votes via /vote
#   4) Start MPC session from LIVE chain snapshot: POST /mpc/sessions/start {"source":"live"}
#   5) DEV mask twice (/mask:server A,B), auto-decrypt, reveal A/B, wait for totals
#   6) Verify totals match expected distribution from VOTES[]
#
# Usage:
#   chmod +x mpc_live_e2e_test_zsh.sh
#   ./mpc_live_e2e_test_zsh.sh
#
# Env overrides (optional):
#   HOST=http://localhost:8080
#   CANDIDATES=4
#   ALLOW_DIRTY_CHAIN=0           # set 1 to skip clean-chain check
#   WORKDIR=verifier
#
set -e
set -u
...
start_live_session() {
  log "Starting MPC session from LIVE chain snapshot"
  curl -fsS "$HOST/mpc/sessions/start" \
    -H 'Content-Type: application/json' \
    -d "$(jq -n --argjson C "$CANDIDATES" '{source:"live", candidates:$C}')" \
    | json > "$WORKDIR/mpc_start_live.json"

  SID=$(jq -r '.session.id' "$WORKDIR/mpc_start_live.json")
